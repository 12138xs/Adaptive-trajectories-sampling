...
def construct_test_set(Eq, NUM_TEST_SAMPLE):
    np.random.seed(512)
    x_test = Eq.interior(NUM_TEST_SAMPLE).requires_grad_(True).to(DEVICE)
    return x_test
...
def train_pipeline():
    ...
    # model.load_state_dict(torch.load('/home/GPU3/xingyu/Ad-DFLM/PINN/10D-031919-PINN'))
    ...
    for step in tqdm(range(NUM_ITERATION+1)):
        ...
        #     plt.show()
        # if step == 20000:
        #     r = Eq.r * np.linspace(0.0, 1.0, 100)
        #     theta = Eq.theta * np.linspace(0.0, 1.0, 100)
        #     r, theta = np.meshgrid(r, theta)
        #     x_grid = r * np.cos(theta)
        #     y_grid = r * np.sin(theta)
        #     points = np.concatenate((x_grid.reshape(-1, 1), y_grid.reshape(-1, 1)), axis=1)
        #     Y_exact = Eq.u_exact(points).cpu().detach().numpy()
        #
        #     Y_pred = model(torch.FloatTensor(points).to(DEVICE))
        #     Y_pred = Y_pred.cpu().detach().numpy()
        #     L1_error = np.abs(Y_exact - Y_pred)
        #     draw.plot_background(Eq.vertexes)
        #     draw.plot_abs_error(x_grid, y_grid, L1_error.reshape(x_grid.shape[0], -1), 'jet')
        ...
        def RAR():
            xn = Eq.interior(1000)
            f_test = Eq.f(xn).detach().reshape(-1, 1)
            loss_int, Ind = loss_interior(Eq, model, xn, f_test)
            index = torch.topk(Ind, k=1, dim=0)[1].cpu().detach().numpy()
            xnew = xn[index].squeeze(1)
            def transit(x, J):
                x = x.cpu().detach().numpy()
                M = x.shape[0]
                D = x.shape[1]
                delta_W = np.random.normal(size=(M * J, D))
                # delta_W = np.reshape(delta_W, (M, m, D))
                x0 = np.expand_dims(x, axis=1)
                x0 = np.broadcast_to(x0, (M, J, D))
                x0 = np.reshape(x0, (M * J, D))
