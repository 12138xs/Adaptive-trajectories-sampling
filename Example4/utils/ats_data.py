...
class ATS():
    def __init__(self, dim, J1, J2, device, type) -> None:
        self.DIMENSION = dim
        self.high = 1
        self.low = -1
    ...
    def source_function(self, x):
        X = np.mean(x, axis=1).reshape(-1, 1)
        return (np.sin(X) - 2) / self.DIMENSION
    ...
    def resample_ats(self, X_train, model, func):
        ...
        if np.any(flag == False):
            X1[flag.squeeze(-1) == False, :] = np.random.uniform(self.low, self.high, (len(X1[flag.squeeze(-1) == False, :]), self.DIMENSION))
