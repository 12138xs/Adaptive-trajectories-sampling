...
class DNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range( self.num_layers ):
            out = self.activation( self.linear[i](out) )
            # res = out
...
class PinnOnePeak:
    ...
    def __init__(self, X_b_train, u_b, img_save_path) -> None:
        ...
        self.iter = 0
        self.net = DNN(Dimension, 20, 1, 7).to(device)
        ...
        self.points = np.random.uniform(-0.1, 0.1, (10000, Dimension))
    ...
    def source_function(self, X):
        x = torch.sum(X*X, axis=1).reshape(-1, 1)
        f = -20*torch.exp(-10*x)*( 20*x - Dimension )
        return f.reshape(-1, 1).detach().requires_grad_(True)
    ...
    def train(self, X_f_train, adam_iters, i = 0):
        ...
        self.net.train()
        self.error = []
        batch_sz  = 5000 # self.x_f.shape[0]
        n_batches = int( np.ceil(self.x_f.shape[0] / batch_sz) )
        # if i >= 1:
        #     freeze_by_idxs(self.net, [0, 1, 2])
        # self.optimizer.step(self.closure)
        for i in range(adam_iters):
            for j in range(n_batches):
                batch_begin = j*batch_sz
                batch_end   = min(j*batch_sz + batch_sz, self.x_f.shape[0])
                x_f_batch = self.x_f[batch_begin:batch_end,]
    ...
    def calculate_total_error(self):
        points = np.random.uniform(-1, 1, (10000, Dimension))
        true_u = self.u_true(points).reshape(100, 100)
        u_predict, _ = self.predict(points, FLAG=False)
        error = np.linalg.norm(u_predict.squeeze() - true_u.flatten(), 1)/np.linalg.norm(true_u.flatten(), 1)
        return error
